...
const ATSAnalyzer = lazy(
  () => import("./components/ATSAnalyzer")
);
const InterviewPrep = lazy(
  () => import("./components/InterviewPrep")
);
const CareerRoadmap = lazy(
  () => import("./components/CareerRoadmap")
);
...
const FEATURE_DESCRIPTIONS = {
  "AI Chat":
    "Ask questions about a resume, a role, or a career path.",

  "Resume Analyzer":
    "Extract structured details from a candidate resume.",

  "ATS Analyzer":
    "Compare a resume to a job description and score fit.",

  "Interview Prep":
    "Create tailored interview questions and prep topics.",

  "Career Roadmap":
    "Build a step-by-step plan for your next role.",
};
...
const FEATURE_META = {
  ...
  "Resume Analyzer": {
    icon: "document",
    kicker: "Extract key details",
  },

  "ATS Analyzer": {
    icon: "target",
    kicker: "Measure role fit",
  },
...
function App() {
  ...
  const [chatDocFile, setChatDocFile] =
    useState(null);
  ...
  const [interviewFile, setInterviewFile] =
    useState(null);
  ...
  const [careerFile, setCareerFile] =
    useState(null);
   ...
   * - Also keeps credentials: include for your existing cookie auth.
   */
  const unifiedFetch = async (
    path,
    options = {}
  ) => {
    setIsLoading(true);
    ...
    try {
      ...
      /*
       * Read the JWT saved during login/register.
       */
      const token =
       ...
       *
       * Your FastAPI backend already supports this
       * through HTTPBearer.
       */
      if (token) {
        headers.set(
      ...
      if (!response.ok) {
        const msg =
          ...
          result?.message ||
          (response.status === 502
            ? "Backend server is not reachable. Please check the backend service."
            : null) ||
          (response.status === 500
      ...
      if (
        ...
        msg =
          "Cannot connect to the server. Please check if both frontend and backend are running.";
      }
   ...
   * Logout.
   *
   * Clear both backend cookie and frontend JWT.
   */
      ...
      {view === "app" && (
        <div className="dashboard-layout">
          ...
          <div className="dashboard-content">
            ...
            <div
              className="dashboard-main panel-fade-in"
              key={activeFeature}
            >
              {activeFeature ===
                "Dashboard" ? (
